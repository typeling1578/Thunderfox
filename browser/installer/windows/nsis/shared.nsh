...
!macro PostUpdate
  ...
  ; install location in the Software\typeling1578 key and uninstall registry entries
  ; that point to our install location for both HKCU and HKLM.
  SetShellVarContext current  ; Set SHCTX to the current user (e.g. HKCU)
  ${RegCleanMain} "Software\typeling1578"
  ${RegCleanUninstall}
  ${UpdateProtocolHandlers}

  ; setup the application model id registration value
  ${InitHashAppModelId} "$INSTDIR" "Software\typeling1578\${AppName}\TaskBarIDs"
  ...
  ${Else}
    ...
    ReadRegStr $0 HKLM "Software\typeling1578\Thunderfox" "CurrentVersion"
    ${If} "$0" != "${GREVersion}"
      WriteRegStr HKLM "Software\typeling1578\Thunderfox" "CurrentVersion" "${GREVersion}"
    ${EndIf}
  ${EndIf}
  ...
  ${If} ${FileExists} "$INSTDIR\postSigningData"
    ...
    Delete /REBOOTOK "$0\typeling1578\Thunderfox\postSigningData"
  ${Else}
    ${If} ${FileExists} "$0\typeling1578\Thunderfox\postSigningData"
      Rename "$0\typeling1578\Thunderfox\postSigningData" "$INSTDIR\postSigningData"
    ${EndIf}
  ${EndIf}
...
!ifdef MOZ_MAINTENANCE_SERVICE
  ...
  ${AndIf} $TmpVal == "HKLM"
    ...
    ${OrIf} ${IsNativeARM64}
      SetRegView 64
    ${EndIf}
    ReadRegDWORD $5 HKLM "Software\typeling1578\MaintenanceService" "Attempted"
    ClearErrors
    ${If} ${RunningX64}
    ${OrIf} ${IsNativeARM64}
      SetRegView lastused
...
${If} $TmpVal == "HKCU"
  ClearErrors
  ReadRegDWORD $0 HKCU "Software\typeling1578\${AppName}\Installer\$AppUserModelID" \
...
!macro SetHandlers
  ...
  ; Associate the file handlers with ThunderfoxHTML, if they aren't already.
  ReadRegStr $6 SHCTX "$0\.htm" ""
  ${WordFind} "$6" "-" "+1{" $6
  ${If} "$6" != "ThunderfoxHTML"
    WriteRegStr SHCTX "$0\.htm"   "" "ThunderfoxHTML$5"
  ${EndIf}

  ReadRegStr $6 SHCTX "$0\.html" ""
  ${WordFind} "$6" "-" "+1{" $6
  ${If} "$6" != "ThunderfoxHTML"
    WriteRegStr SHCTX "$0\.html"  "" "ThunderfoxHTML$5"
  ${EndIf}

  ReadRegStr $6 SHCTX "$0\.shtml" ""
  ${WordFind} "$6" "-" "+1{" $6
  ${If} "$6" != "ThunderfoxHTML"
    WriteRegStr SHCTX "$0\.shtml" "" "ThunderfoxHTML$5"
  ${EndIf}

  ReadRegStr $6 SHCTX "$0\.xht" ""
  ${WordFind} "$6" "-" "+1{" $6
  ${If} "$6" != "ThunderfoxHTML"
    WriteRegStr SHCTX "$0\.xht"   "" "ThunderfoxHTML$5"
  ${EndIf}

  ReadRegStr $6 SHCTX "$0\.xhtml" ""
  ${WordFind} "$6" "-" "+1{" $6
  ${If} "$6" != "ThunderfoxHTML"
    WriteRegStr SHCTX "$0\.xhtml" "" "ThunderfoxHTML$5"
  ${EndIf}
  ...
  ${AddAssociationIfNoneExist} ".oga" "ThunderfoxHTML$5"
  ${AddAssociationIfNoneExist} ".ogg" "ThunderfoxHTML$5"
  ${AddAssociationIfNoneExist} ".ogv" "ThunderfoxHTML$5"
  ${AddAssociationIfNoneExist} ".webm" "ThunderfoxHTML$5"
  ${AddAssociationIfNoneExist} ".svg" "ThunderfoxHTML$5"
  ${AddAssociationIfNoneExist} ".webp"  "ThunderfoxHTML$5"
  ${AddAssociationIfNoneExist} ".avif" "ThunderfoxHTML$5"

  ${AddAssociationIfNoneExist} ".pdf" "ThunderfoxPDF$5"

  ; An empty string is used for the 5th param because ThunderfoxHTML- is not a
  ; protocol handler.  Ditto for ThunderfoxPDF-.
  ${AddDisabledDDEHandlerValues} "ThunderfoxHTML$5" "$2" "$8,${IDI_DOCUMENT_ZERO_BASED}" \
                                 "${AppRegName} HTML Document" ""

  ${AddDisabledDDEHandlerValues} "ThunderfoxPDF$5" "$2" "$8,${IDI_DOCUMENT_PDF_ZERO_BASED}" \
                                 "${AppRegName} PDF Document" ""

  ${AddDisabledDDEHandlerValues} "ThunderfoxURL$5" "$2" "$8,${IDI_DOCUMENT_ZERO_BASED}" "${AppRegName} URL" \
                                 "true"
...
!macro SetStartMenuInternet RegKey
  ...
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".htm"   "ThunderfoxHTML$2"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".html"  "ThunderfoxHTML$2"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".shtml" "ThunderfoxHTML$2"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".xht"   "ThunderfoxHTML$2"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".xhtml" "ThunderfoxHTML$2"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".svg"   "ThunderfoxHTML$2"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".webp"  "ThunderfoxHTML$2"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".avif"  "ThunderfoxHTML$2"

  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".pdf"   "ThunderfoxPDF$2"
  ...
  WriteRegStr ${RegKey} "$0\Capabilities\URLAssociations" "http"   "ThunderfoxURL$2"
  WriteRegStr ${RegKey} "$0\Capabilities\URLAssociations" "https"  "ThunderfoxURL$2"
  WriteRegStr ${RegKey} "$0\Capabilities\URLAssociations" "mailto" "ThunderfoxURL$2"
...
!ifdef HAVE_64BIT_BUILD
  ...
  ClearErrors
  ReadRegDWORD $2 HKLM "Software\typeling1578\${AppName}\32to64DidMigrate" "$1"
  ...
  ${AndIf} $2 != 1
    ClearErrors
    WriteRegDWORD HKLM "Software\typeling1578\${AppName}\32to64DidMigrate" "$1" 1
    ${If} ${Errors}
      ; There was an error writing to HKLM so just write it to HKCU
      WriteRegDWORD HKCU "Software\typeling1578\${AppName}\32to64DidMigrate" "$1" 1
    ${Else}
      ; This will delete the value from HKCU if it exists
      DeleteRegValue HKCU "Software\typeling1578\${AppName}\32to64DidMigrate" "$1"
    ${EndIf}
  ${EndIf}

  ClearErrors
  ReadRegDWORD $2 HKCU "Software\typeling1578\${AppName}\32to64DidMigrate" "$1"
  ; If there were no errors then the system was updated from Firefox 32 bit to
  ; Firefox 64 bit and if the value is already 1 then the registry value has
...
!else
  ...
  ${OrIf} ${IsNativeARM64}
    ...
    ${If} ${Errors}
      ClearErrors
      WriteRegDWORD HKLM "Software\typeling1578\${AppName}\32to64DidMigrate" "$1" 0
      ; If there were errors write the value in HKCU.
      ${If} ${Errors}
        WriteRegDWORD HKCU "Software\typeling1578\${AppName}\32to64DidMigrate" "$1" 0
      ${EndIf}
    ${EndIf}
  ${EndIf}
...
; The IconHandler reference for ThunderfoxHTML can end up in an inconsistent state
; due to changes not being detected by the IconHandler for side by side
; installs (see bug 268512). The symptoms can be either an incorrect icon or no
; icon being displayed for files associated with Firefox (does not use SHCTX).
!macro FixShellIconHandler RegKey
  ; Find the correct key to update, either ThunderfoxHTML or ThunderfoxHTML-[PathHash]
  StrCpy $3 "ThunderfoxHTML-$AppUserModelID"
  ClearErrors
  ReadRegStr $0 ${RegKey} "Software\Classes\$3\DefaultIcon" ""
  ${If} ${Errors}
    StrCpy $3 "ThunderfoxHTML"
  ${EndIf}
...
; Add Software\typeling1578\ registry entries (uses SHCTX).
!macro SetAppKeys
  ...
  StrCpy $0 "Software\typeling1578\${BrandFullNameInternal}\${AppVersion}$3 (${ARCH} ${AB_CD})\Main"
  ${WriteRegStr2} $TmpVal "$0" "Install Directory" "$8" 0
  ${WriteRegStr2} $TmpVal "$0" "PathToExe" "$8\${FileMainEXE}" 0

  StrCpy $0 "Software\typeling1578\${BrandFullNameInternal}\${AppVersion}$3 (${ARCH} ${AB_CD})\Uninstall"
  ${WriteRegStr2} $TmpVal "$0" "Description" "${BrandFullNameInternal} ${AppVersion}$3 (${ARCH} ${AB_CD})" 0

  StrCpy $0 "Software\typeling1578\${BrandFullNameInternal}\${AppVersion}$3 (${ARCH} ${AB_CD})"
  ...
  StrCpy $0 "Software\typeling1578\${BrandFullNameInternal} ${AppVersion}$3\bin"
  ${WriteRegStr2} $TmpVal "$0" "PathToExe" "$8\${FileMainEXE}" 0

  StrCpy $0 "Software\typeling1578\${BrandFullNameInternal} ${AppVersion}$3\extensions"
  ${WriteRegStr2} $TmpVal "$0" "Components" "$8\components" 0
  ${WriteRegStr2} $TmpVal "$0" "Plugins" "$8\plugins" 0

  StrCpy $0 "Software\typeling1578\${BrandFullNameInternal} ${AppVersion}$3"
  ...
  StrCpy $0 "Software\typeling1578\${BrandFullNameInternal}$3"
...
!macro SetUninstallKeys
  ...
  ${If} $2 == ""
    ...
    ${WriteRegStr2} $1 "$0" "Publisher" "typeling1578" 0
...
; to ThunderfoxHTML when there is a value for PersistentHandler or by removing the
...
!macro FixBadFileAssociation FILE_TYPE
  ...
  ${If} "$2" != ""
    ; Since there is a persistent handler remove ThunderfoxHTML as the default
    ; value from both HKCU and HKLM if it set to ThunderfoxHTML.
    ${If} "$0" == "ThunderfoxHTML"
    ...
    ${EndIf}
  ${ElseIf} "$0" == "ThunderfoxHTML"
    ; Since HKCU is set to ThunderfoxHTML remove ThunderfoxHTML as the default value
...
!macro UpdateProtocolHandlers
  ...
  ${IsHandlerForInstallDir} "ThunderfoxHTML-$AppUserModelID" $R9
  ${If} "$R9" == "true"
    ; An empty string is used for the 5th param because ThunderfoxHTML is not a
    ; protocol handler.
    ${AddDisabledDDEHandlerValues} "ThunderfoxHTML-$AppUserModelID" "$2" "$8,${IDI_DOCUMENT_ZERO_BASED}" \
                                   "${AppRegName} HTML Document" ""
  ${Else}
    ${IsHandlerForInstallDir} "ThunderfoxHTML" $R9
    ${If} "$R9" == "true"
      ${AddDisabledDDEHandlerValues} "ThunderfoxHTML" "$2" "$8,${IDI_DOCUMENT_ZERO_BASED}" \
                                     "${AppRegName} HTML Document" ""
    ${EndIf}
  ${EndIf}

  ; ThunderfoxPDF-* was added after ThunderfoxHTML and ThunderfoxURL, so we've never
  ; supported bare "ThunderfoxPDF".  But we won't have it from the installer, so we
  ...
  ${AddDisabledDDEHandlerValues} "ThunderfoxPDF-$AppUserModelID" "$2" "$8,${IDI_DOCUMENT_PDF_ZERO_BASED}" \
                                 "${AppRegName} PDF Document" ""

  ${IsHandlerForInstallDir} "ThunderfoxURL-$AppUserModelID" $R9
  ${If} "$R9" == "true"
    ${AddDisabledDDEHandlerValues} "ThunderfoxURL-$AppUserModelID" "$2" "$8,${IDI_DOCUMENT_ZERO_BASED}" \
                                   "${AppRegName} URL" "true"
  ${Else}
    ${IsHandlerForInstallDir} "ThunderfoxURL" $R9
    ${If} "$R9" == "true"
      ${AddDisabledDDEHandlerValues} "ThunderfoxURL" "$2" "$8,${IDI_DOCUMENT_ZERO_BASED}" \
                                     "${AppRegName} URL" "true"
    ${EndIf}
  ${EndIf}
...
!macro AddMaintCertKeys
  ...
  ${If} $R0 != ""
    ...
    WriteRegDWORD HKLM "Software\typeling1578\MaintenanceService" "Attempted" 1
...
!macro RemoveDeprecatedKeys
  ...
  DeleteRegKey HKLM "Software\Classes\Thunderfox.URL"
  DeleteRegKey HKCU "Software\Classes\Thunderfox.URL"
!macroend
...
!macro MigrateTaskBarShortcut
  ...
  ${If} ${FileExists} "$0"
    ...
    ${If} ${Errors}
      ...
      WriteRegDWORD HKCU \
        "Software\typeling1578\${AppName}\Installer\$AppUserModelID" \
      ...
      ${EndIf}
    ${ElseIf} ${AtLeastWin10}
      ${GetInstallerRegistryPref} "Software\typeling1578\${AppName}" \
