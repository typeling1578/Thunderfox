...
Section "-InstallStartCleanup"
  ...
  ${CleanUpdateDirectories} "typeling1578\Thunderfox" "typeling1578\updates"
...
Section "-Application" APP_IDX
  ...
  ; On install always add the ThunderfoxHTML-, ThunderfoxPDF-, and ThunderfoxURL- keys.
  ; An empty string is used for the 5th param because ThunderfoxHTML- is not a
  ; protocol handler.
  ${GetLongPath} "$INSTDIR\${FileMainEXE}" $8
  StrCpy $2 "$\"$8$\" -osint -url $\"%1$\""

  ; In Win8, the delegate execute handler picks up the value in ThunderfoxURL- and
  ; ThunderfoxHTML- to launch the desktop browser when it needs to.
...
!ifdef MOZ_DEFAULT_BROWSER_AGENT
  ...
  WriteRegDWORD HKCU "Software\typeling1578\${AppName}\Installer\$AppUserModelID" \
...
Section "-InstallEndCleanup"
  ...
  ${Unless} ${Silent}
    ...
    ${If} "$0" == "1"
      ...
      ${If} "$0" != ""
        ClearErrors
        WriteRegStr HKCU "Software\typeling1578\Thunderfox" "OldDefaultBrowserCommand" "$0"
      ${EndIf}
    ...
    ${ElseIfNot} ${Errors}
      ...
      ClearErrors
      WriteRegStr HKCU "Software\typeling1578\Thunderfox" "DefaultBrowserOptOut" "True"
      ${If} ${Errors}
...
Function SendPing
  ...
  ClearErrors
  WriteRegStr HKLM "Software\typeling1578" "${BrandShortName}InstallerTest" \
                   "Write Test"
  ${If} ${Errors}
  ...
  ${If} $0 != ""
    ...
    ${Else}
      StrCpy $0 "$0" "" -14 # 14 == length of "thunderfox.exe"
      ${If} "$0" == "${FileMainEXE}"
...
Function preComponents
  ...
  ClearErrors
  WriteRegStr HKLM "Software\typeling1578" \
...
!endif
  ...
  SetShellVarContext all
  ${GetFirstInstallPath} "Software\typeling1578\${BrandFullNameInternal}" $0
  ${If} "$0" == "false"
    SetShellVarContext current
    ${GetFirstInstallPath} "Software\typeling1578\${BrandFullNameInternal}" $0
    ${If} "$0" == "false"
