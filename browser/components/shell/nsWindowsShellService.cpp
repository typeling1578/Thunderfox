...
nsWindowsShellService::CheckAllProgIDsExist(bool* aResult) {
  ...
  *aResult =
      CheckProgIDExists(FormatProgID(L"ThunderfoxURL", aumid.get()).get()) &&
      CheckProgIDExists(FormatProgID(L"ThunderfoxHTML", aumid.get()).get()) &&
      CheckProgIDExists(FormatProgID(L"ThunderfoxPDF", aumid.get()).get());
  return NS_OK;
}
...
nsWindowsShellService::SetDesktopBackground(dom::Element* aElement,
  ...
  // eventually, the path is "%APPDATA%\typeling1578\Thunderfox\Desktop Background.bmp"
