...
add_task(async function setup_preferences() {
  await SpecialPowers.pushPrefEnv({
    set: [
      ["browser.policies.alternateGPO", "SOFTWARE\\typeling1578\\PolicyTesting"],
    ],
  });
});
...
add_task(async function test_gpo_policies() {
  ...
  let regLocation =
    "SOFTWARE\\typeling1578\\PolicyTesting\\typeling1578\\" + Services.appinfo.name;
  ...
  wrk.open(wrk.ROOT_KEY_CURRENT_USER, "SOFTWARE\\typeling1578", wrk.ACCESS_WRITE);
  wrk.removeChild("PolicyTesting\\typeling1578\\" + Services.appinfo.name);
  wrk.removeChild("PolicyTesting\\typeling1578");
